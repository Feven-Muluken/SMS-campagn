...
const getLoginErrorDetails = (err) => {
  if (!err?.response) {
    if (err?.code === 'ECONNABORTED') {
      return { message: 'Login request timed out. Please try again.', fieldErrors: {} };
    }
    return { message: 'Unable to reach server. Check your internet connection and try again.', fieldErrors: {} };
  }
  ...
  if (status === 400) {
    return { message: data.message || 'Please review the form and try again.', fieldErrors };
  }
  if (status === 401) {
    return { message: data.message || 'Invalid email or password.', fieldErrors: {} };
  }
  if (status === 403) {
    return { message: data.message || 'Your account is not allowed to sign in here.', fieldErrors: {} };
  }
  if (status === 429) {
    return { message: data.message || 'Too many login attempts. Please wait and try again.', fieldErrors: {} };
  }
  if (status >= 500) {
    return { message: data.message || 'Server error while logging in. Please try again shortly.', fieldErrors: {} };
  }

  return { message: data.message || 'Login failed. Please try again.', fieldErrors: {} };
};
...
const Auth = () => {
  ...
  const { refreshUser, setCompanyContext } = useUser();
...
const handleLogin = async (e) => {
  ...
  if (clientFieldErrors.email || clientFieldErrors.password) {
    setLoginFieldErrors(clientFieldErrors);
    setLoginError('Please fix the highlighted fields and try again.');
    return;
  }
  ...
  try {
    ...
    setCompanyContext({
      activeCompanyId: res.data?.activeCompanyId || decoded.activeCompanyId || null,
      companyRole: res.data?.companyRole || decoded.companyRole || null,
      companyPermissions: res.data?.companyPermissions || decoded.companyPermissions || [],
    ...
    if (decoded.role === 'admin') {
      ...
      const activeId = res.data?.activeCompanyId || decoded.activeCompanyId;
      if (!activeId) {
        toast('No company linked', {
          description: 'You can open My profile after sign-in. A super admin must assign you to a company for full access.',
        });
      }
    ...
    const { message, fieldErrors } = getLoginErrorDetails(err);
                ...
                <form onSubmit={handleLogin} className="space-y-6">
                  ...
                  <div className="relative">
                    ...
                    {loginFieldErrors.email ? (
                      <p className="text-xs mt-2" style={{ color: '#B91C1C' }}>{loginFieldErrors.email}</p>
                    ) : null}
                  </div>
                  ...
                  <div className='relative'> 
                    ...
                    {loginFieldErrors.password ? (
                      <p className="text-xs mt-2" style={{ color: '#B91C1C' }}>{loginFieldErrors.password}</p>
                    ) : null}
                  </div>
                  ...
                  <div className='mt-10'>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        opacity: isLoginSubmitting ? 0.8 : 1,
                        cursor: isLoginSubmitting ? 'not-allowed' : 'pointer',
                      }}
                    >
