  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context
          ...
          .load(reviewQueue: _isPlatformAdmin);
    });
  }

  Future<void> _review(String id, String status) async {
    setState(() => _reviewingId = id);
    final error = await context.read<SenderIdProvider>().review(id, status);
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () => provider.load(reviewQueue: isPlatformAdmin),
        child: ListView(
          ...
          children: [
            ...
            AnimatedSwitcher(
              ...
              child: provider.loading
                  ? const Padding(
                      padding: EdgeInsets.only(top: 80), child: Loader())
                  : provider.error != null
                      ? ErrorView(
                          message: provider.error!,
                          onRetry: () => provider.load(
                              reviewQueue: isPlatformAdmin))
                                          ...
                                          ? Card(
                                              child: Padding(
                                                ...
                                                child: Column(
                                                  ...
                                                  children: [
                                                    Text(provider.requests[i].senderId,
                                                        style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w700)),
                                                    const SizedBox(height: 4),
                                                    Text('${provider.requests[i].companyName ?? 'Unknown company'} · ${provider.requests[i].requesterName ?? 'Unknown requester'}'),
                                                    if (provider.requests[i].reason?.isNotEmpty == true)
                                                      Padding(
                                                        padding: const EdgeInsets.only(top: 4),
                                                        child: Text(provider.requests[i].reason!),
                                                      ),
                                                    const SizedBox(height: 10),
                                                    Row(
                                                      children: [
                                                        Expanded(child: OutlinedButton(
                                                          onPressed: _reviewingId == null ? () => _review(provider.requests[i].id, 'rejected') : null,
                                                          child: const Text('Reject'),
                                                        )),
                                                        const SizedBox(width: 8),
                                                        Expanded(child: FilledButton.icon(
                                                          onPressed: _reviewingId == null ? () => _review(provider.requests[i].id, 'approved') : null,
                                                          icon: _reviewingId == provider.requests[i].id
